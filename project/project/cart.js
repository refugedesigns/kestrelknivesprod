  ...
  items: [
    {
      ...
      handleColor: 'Orange',
      sku: 'OVS-001',
      image: 'https://images.pexels.com/photos/4226769/pexels-photo-4226769.jpeg?auto=compress&cs=tinysrgb&w=200'
    }
  ],
  subtotal: 135.00,
  tax: 13.50,
...
function updateCartTotals() {
  ...
  // Update display
  document.querySelector('[data-subtotal]').textContent = `GH¢${subtotal.toFixed(2)}`;
...
function updateCart() {
  ...
  setTimeout(() => {
    ...
    button.classList.remove('border-gray-300', 'hover:border-gray-400', 'text-gray-700');
    button.classList.add('border-green-500', 'text-green-600');
    
    setTimeout(() => {
      button.textContent = originalText;
      button.disabled = false;
      button.classList.add('border-gray-300', 'hover:border-gray-400', 'text-gray-700');
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const subtotalElement = document.querySelector('.flex.justify-between .font-medium.text-gray-900');
  const taxElement = document.querySelectorAll('.flex.justify-between .font-medium.text-gray-900')[1];
  const totalElement = document.querySelector('.text-lg.font-bold.text-gray-900');
